...
#   SETTLE=25 tools/blender/field/shoot.sh <udid> <out dir> [shots...]
cd "$(dirname "$0")/../../.."
...
xcrun simctl spawn "$UDID" log show --last 30m --style compact \
  --predicate 'subsystem == "com.mutaaf.fantasyedge"' 2>/dev/null | grep -i "shadergraph\|field:" > "$OUT/shadergraph.log"
