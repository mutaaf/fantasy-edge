...
- **A weekend's capture is committed in three parts, and sampled in the fourth.** The boards, the final summaries and the logs go into git whole - they are the night, and a replay needs them. The per-game live snapshots do not: 19 September's came to 92 MB across 54 games. Commit the snapshots of the six games the recorder's own leverage rule watched most (rank every recorded board with `cfb.leverage` and count frames inside the live top six), write `live/README.md` saying which and why, and leave the rest on disk. A dropped game loses nothing permanent: its final carries every play and `reconstruct.py` rebuilds any moment of it.
- **Derived frames are rebuilt, not stored.** A backfill's `scoreboard/` is built from its summaries, so it is gitignored like `*-merged/`; the summaries and the reference board it is built from are committed. Rebuild with `python3 tools/backfill_slate.py frames --slate <date>`, then `python3 tools/merge_capture.py --slate <date>`. `tests/test_sample.py` does exactly that from a tree of only what git tracks, so the day this stops working a test says so.
- **A sampled capture says so in the payload.** Every tile carries `detail`: `available`, `afterFinal` when the play-by-play only arrives with the final, or `unavailable`. A tile must never look complete and then open onto nothing.
