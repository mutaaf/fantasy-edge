        ...
        VStack(alignment: .leading, spacing: compact ? 10 : 13) {
            HStack(spacing: 9) {
                ForEach(0..<14, id: \.self) { _ in
                    Circle().fill(Tokens.lightBank).frame(width: 9, height: 9)
                        .shadow(color: Tokens.lightBank.opacity(0.7), radius: 5)
                }
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                StatusLine(game: game, fontSize: 15)
            }
            VStack(spacing: 10) {
                BigTeam(game: game, side: game.away, compact: compact)
                BigTeam(game: game, side: game.home, compact: compact)
            }
