...
/// The procedural bowl the kit replaced: stepped tiers, aisles and rails
/// drawn from the scene. The Bowl actor falls back to it when a model is
/// missing from the bundle, so a checkout without the kit still has stands.
extension BowlActor {
    func buildProcedural(_ c: StadiumContext) {
        clear()
